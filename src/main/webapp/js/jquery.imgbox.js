...
;(function ($) {
  ...
  function draw(pos) {
    ...
    $('#imgbox-img-' + nr).css('width', Math.round(width - ((((width - Math.min(orig_pos.width, final_pos.width)) * 100) / (Math.max(orig_pos.width - final_pos.width, final_pos.width - orig_pos.width)) / 100))) + 'px');
  ...
  function getZoomTo() {
    ...
    var horizontal_space = 20;//(opts.padding + margin) * 2;
    var vertical_space = 20;//(opts.padding + margin) * 2;
